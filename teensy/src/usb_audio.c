	...
	while (length > 0) {
		if (!tx_packet) {
			while (1) {
				...
				if (usb_tx_packet_count(AUDIO_TX_ENDPOINT) < TX_PACKET_LIMIT) {
		...
		len = AUDIO_TX_SIZE - tx_packet->index;
		if (len > length) {
			len = length;
		}
		...
		if (tx_packet->index >= AUDIO_TX_SIZE) {
			tx_packet->len = AUDIO_TX_SIZE;
			usb_tx(AUDIO_TX_ENDPOINT, tx_packet);
			tx_packet = NULL;
		}
...
uint8_t usb_audio_read_data(const void* data, uint8_t length)
{
	...
	while (length > 0) {
		...
		if (!rx_packet) {
rx:
			rx_packet = usb_rx(AUDIO_RX_ENDPOINT);
			if (!rx_packet) {
				break;
			}
			if (rx_packet->len == 0) {
...
void usb_audio_flush_callback(void)
{
	...
	if (tx_packet) {
		...
		if (tx) {
			usb_tx(AUDIO_TX_ENDPOINT, tx);
		} else {
			usb_audio_transmit_flush_timer = 1;
		}
	}
}
